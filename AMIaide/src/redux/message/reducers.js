...
const messageCreateRequest = (state = initialState) => (
  {...state, loading: true, createSuccess: false}
)
...
const messageCreateSuccess = (state = initialState, action) => {
  ...
  return {...state, list: data, loading: false, createSuccess: true, hasMore: messages.length > 0};
}

const messageCreateFailure = (state = initialState)  => (
  {...state, loading: false, createSuccess: false}
)
