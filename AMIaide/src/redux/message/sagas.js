...
function* messagesRequest({filters}) {
  ...
  const now = moment()
  const start = now.subtract(5, 'd').format('YYYY-MM-DD')
  const end = now.add(5, 'd').format('YYYY-MM-DD')
  
  console.log(`start: ${start}`)
  console.log(`end: ${end}`)

  const [error, response] =  yield call(MessagesService.messageBetween, {startDate: start, endDate: end});
  console.log(error)
...
function* messageCreateRequest({data}) {
  ...
  const {user, activity, subject, range_date, imgURL, locate, moment_diffuse, time_diffuse, reccurence} = data.message
  let message = {
    auxiliary_id: 2,
    helped_user_id: 2,
    activite: activity,
  ...
  videoFormData.append("file", fileVideo.data, `testounet.${mime.extension(contentType)}`);
