...
function* auxiliariesCreateRequest({filters}) {
  ...
  const [error, response] =  yield call(AuxiliariesService.createAuxiliary, filters);
  console.log(error)
...
function* auxiliariesRequest({filters}) {
  ...
  console.log(error)
  console.log(response)
  if (!error && response) yield put(AuxiliaryAction.auxiliariesSuccess(response.auxiliaries));
  else {
...
function* auxiliariesModifyRequest({auxiliary}) {
  ...
  const aux = auxiliary
  const [error, response] =  yield call(AuxiliariesService.modifyAuxiliary, {id, aux});
  console.log(error)
  console.log(response)
