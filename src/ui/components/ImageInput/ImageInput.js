...
const ImageInput = ({ source, onChangeImage }) => {
  ...
  const emptyState = () => {
    return (
      ...
      >
        ...
        <Subheading style={{ color: '#FFF', fontSize: 18 }}>
          Pour ajouter une image cliquer ici.
        </Subheading>
      </View>
    )
  }
  ...
  return (
    <TouchableRipple onPress={_showImagePicker}>
      <View>
        <Subheading style={{ color: '#AAA', fontSize: 13 }}>
          AJOUTER UNE IMAGE
        </Subheading>
        {img == null && emptyState()}
        {img && (
