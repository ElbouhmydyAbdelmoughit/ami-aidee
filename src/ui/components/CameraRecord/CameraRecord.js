...
const CameraRecord = ({ style, form, source, onValidate }) => {
  ...
  const retake = () => {
    logActivity('press_retake_video_btn')
    setUri({ uri: null })
  }

  const validate = () => {
    logActivity('press_validate_video_btn')
    onValidate(uri)
  }
  ...
  const renderPreviewAction = () => {
    return (
      ...
      >
        ...
        <Button onPress={() => validate()} mode="contained">
          Valider
        </Button>
      </View>
    )
      ...
      >
        {isSettingUp ? (
          ...
          <div>
            <TouchableRipple
              onPress={() => {
                logActivity('press_record_video_btn')
                updateRecord()
              }}
