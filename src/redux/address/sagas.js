...
function* addressesRequest({filters}) {
  ...
  if (!error && response) yield put(AddressAction.addressesSuccess(response.addresses));
  else {
...
function* addressesModifyRequest({address}) {
  ...
  yield put(LoaderActions.loading())
  const id = address.id
  const [error, response] =  yield call(AddressesService.modifyAddress, {id, address});
  console.log(error)
  console.log(response)
