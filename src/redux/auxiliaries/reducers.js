...
const auxiliariesSuccess = (state = initialState, action) => {
  ...
  return {...state, list: {...list, ...data}, loading: false, hasMore: auxiliaries.length > 0};
};

const auxiliaryCreateRequest = (state = initialState) => (
  {...state, loading: true, auxiliaryCreateSuccess: false}
)
const auxiliaryCreateSuccess = (state = initialState, action) => {
  ...
  return {...state, list: data, loading: false, hasMore: auxiliaries.length > 0};
 };
const auxiliaryCreateFailure = (state = initialState)  => (
  {...state, loading: false, auxiliaryCreateSuccess: false}
)

const auxiliaryModifyRequest = (state = initialState) => (
  {...state, loading: true, auxiliaryModifySuccess: false}
)
...
const auxiliaryModifyFailure = (state = initialState)  => (
  {...state, loading: false, auxiliaryModifySuccess: false}
)

const auxiliaryDeleteRequest = (state = initialState) => (
  {...state, loading: true, auxiliaryDeleteSuccess: false}
)
...
const auxiliaryDeleteFailure = (state = initialState)  => (
  {...state, loading: false, auxiliaryDeleteSuccess: false}
)
